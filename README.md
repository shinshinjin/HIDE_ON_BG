...
시작 화면 또는 Host 대기실의 ‘게임 선택’에서 **Bluff · 2–6명**을 선택하세요. 컴퓨터와 혼자 플레이도 가능합니다. 게임을 바꾸면 참가자는 다시 준비해야 합니다. 기존 Yacht Dice는 그대로 2–8명을 지원합니다.

비공개 주사위, 원작 보드 순서의 선언, BLUFF 공개 판정, Exact, 주사위 감소와 탈락, 최후 1명 승리를 지원합니다. [채택 규칙·비공개 정보 경계·온라인 예외·검증](docs/BLUFF.md)을 확인하세요. 기존 방·메모·저장·재접속을 사용하며 추가 외부 서비스나 키는 없습니다. 배포 후 모든 참가자가 새로고침해야 최신 통신 버전으로 접속할 수 있습니다.
