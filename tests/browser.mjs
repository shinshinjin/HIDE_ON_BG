...
try{
 ...
 const guests=[];for(let i=1;i<=7;i++){const p=await page('참가 '+i,i%2?390:1280);await p.locator('#invite').fill(code);await click(p,'join');await until(()=>p.locator('[data-action="ready"]').isVisible());await click(p,'ready');guests.push(p);console.log('Guest ready',i);}
