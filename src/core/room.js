...
export function createRoom(host,title,mode='multi',code='',gameId='yacht'){getGame(gameId);return {version:1,gameId,id:uid(),code,title:name(title,40),mode,hostId:host.id,revision:0,phase:'lobby',players:[{id:host.id,name:name(host.name),ready:true,online:true}],game:null,messages:[],createdAt:Date.now(),updatedAt:Date.now()};}
...
export function act(room,actor,action){
 ...
 if(action.type==='chat'){const text=typeof action.text==='string'?action.text.trim():'';if(!text||text.length>400||/[\u0000-\u0008\u000b\u000c\u000e-\u001f]/.test(text))throw Error('메모는 1~400자로 입력하세요.');r.messages.push({id:uid(),sender:p.name,text,time:Date.now(),system:false});r.messages=r.messages.slice(-150);}
  ...
  if(actor!==r.hostId||r.phase!=='lobby')throw Error('대기실 Host만 게임을 선택할 수 있습니다.');const selected=getGame(action.gameId);if(r.players.length>selected.maxPlayers)throw Error(`${selected.name}는 최대 ${selected.maxPlayers}명입니다.`);r.gameId=selected.id;for(const q of r.players)q.ready=q.id===r.hostId;system(r,`${selected.name} 선택 · 참가자는 다시 준비해주세요.`);
 }else if(action.type==='start'){
  if(actor!==r.hostId||r.phase!=='lobby'||r.players.length<def.minPlayers||r.players.length>def.maxPlayers||r.players.some(p=>!p.ready||!p.online))throw Error('Host만 모든 참가자의 준비 완료 후 시작할 수 있습니다.');
  ...
  for(const message of def.actionLog?.(r.game,actor,action,r.players)||[])system(r,message);
  if(r.gameId==='yacht'&&action.type==='score'){const h=r.game.history.at(-1);system(r,`${p.name} · ${def.categories.find(c=>c.id===h.category).label} ${h.value}점`);}
