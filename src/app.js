...
async function restore(value){if(s.room)throw Error('먼저 현재 문서를 닫아주세요.');const data=validateSave(value);const r=data.room;s.self={...identity,...r.players.find(p=>p.id===r.hostId)};await lock(s.self.id);tokens=data.tokens;r.players.forEach(p=>{p.online=p.id===r.hostId||!!p.bot;});system(r,'저장 문서를 복구했습니다. 참가자의 재접속을 기다립니다.');touch(r);if(r.mode==='multi'){net=network();await net.host(r,tokens);}else{s.status='로컬 · 저장 문서 복구됨';updateRoom(r);await persist();}}
